...
| エージェント | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.view.set`、`agent.view.clear`、`agent.provider.replace`、`agent.provider.clear`、`agent.provider.get`、`agent.provider.snapshot`、`agent.provider.focus`、`agent.provider.focused` |
...
エージェントプロバイダーを使うと、リモートまたは仮想エージェントを、各エージェント用の
ローカルワークスペース、ペイン、PTY を作らずに組み込みの Agents ビューへ追加できます。
プロバイダーのレコードには、ローカルエージェントと同じ status、メタデータトークン、
フィルター、並び順、オーケストレーションカードが適用されます。`agent.list` は物理的な
ローカルエージェントだけを返すため、別のプロバイダーの投影を再取り込みしません。

`agent.provider.replace` は、一つの source の完全なスナップショットをアトミックに
置き換えます。revision は 1 から始め、単調に増加させます。viewer には存在する
ローカルペインを指定します。一つの source に登録できるレコードは最大 2,000 件です。
各レコードでは state label を 8 個、メタデータトークンを 32 個まで指定できます。
メタデータキーは 32 文字以内の ASCII 英数字、アンダースコア、ハイフンに限られ、値は
256 文字以内です:

```json
{"id":"provider_replace","method":"agent.provider.replace","params":{"source":"fleet:default","revision":12,"viewer":{"workspace_id":"w1","pane_id":"w1:p1"},"agents":[{"id":"linux/reviewer","name":"linux · reviewer","agent":"codex","agent_status":"working","tokens":{"orchestration_id":"backlog:default","repository":"example","responsibility":"Review issue 302"}}]}}
```

`agent.provider.get` は一つの source/id レコードを返し、
`agent.provider.snapshot` は source 全体のスナップショットを返します。source を消去する
場合は、より大きい tombstone revision を送信します。消去後もスナップショットは取得でき、
revision を保持したまま viewer を省略し、空の `agents` リストを返します:
...
`agent.provider.focus` は source で共有する viewer ペインをフォーカスし、サイドバーでは
選択したプロバイダーの識別子を維持します。`agent.provider.focused` はその識別子を返します。
選択変更に合わせて viewer を切り替えるには `agent.provider_focused` を購読します。viewer が
フォーカスを失うか、選択中のレコードが消えると、イベントの `target` は `null` になります。

プロバイダーのスナップショットは一時的な状態です。サーバーをコールドスタートした後は
再登録してください。`ping` レスポンスの `capabilities.agent_provider` が対応状況を示します。
