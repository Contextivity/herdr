...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.view.set`, `agent.view.clear`, `agent.provider.replace`, `agent.provider.clear`, `agent.provider.get`, `agent.provider.snapshot`, `agent.provider.focus`, `agent.provider.focused` |
...
## Data-only agent providers

An agent provider adds remote or synthetic agents to the native Agents view
without creating one local workspace, pane, or PTY for each agent. Provider
records use the same status, metadata tokens, filters, ordering, and
orchestration cards as local agents. `agent.list` continues to return only
physical local agents, which prevents one provider from ingesting another
provider's projection.

`agent.provider.replace` atomically replaces one source's complete snapshot.
The revision starts at 1 and must increase. The viewer must identify one live
local pane. A source may contain at most 2,000 records; each record accepts up
to 8 state labels and 32 metadata tokens. Metadata keys use at most 32 ASCII
letters, digits, underscores, or hyphens, and values use at most 256 characters:

```json
{
  "id": "provider_replace",
  "method": "agent.provider.replace",
  "params": {
    "source": "fleet:default",
    "revision": 12,
    "viewer": {"workspace_id": "w1", "pane_id": "w1:p1"},
    "agents": [{
      "id": "linux/reviewer",
      "name": "linux · reviewer",
      "agent": "codex",
      ...
      "tokens": {
        "orchestration_id": "backlog:default",
        "repository": "example",
        "responsibility": "Review issue 302"
      }
    }]
  }
}
...
Use `agent.provider.get` for one source/id pair and
`agent.provider.snapshot` for the current source snapshot. Clear a source with
a newer tombstone revision. The snapshot remains readable after clear and
returns that revision with no viewer and an empty `agents` list:

```json
{"id":"provider_get","method":"agent.provider.get","params":{"source":"fleet:default","id":"linux/reviewer"}}
{"id":"provider_snapshot","method":"agent.provider.snapshot","params":{"source":"fleet:default"}}
{"id":"provider_clear","method":"agent.provider.clear","params":{"source":"fleet:default","revision":13}}
```

`agent.provider.focus` focuses the source's shared viewer pane while preserving
the selected provider identity in the sidebar. `agent.provider.focused` returns
that identity. Subscribe to `agent.provider_focused` to retarget the viewer when
selection changes; its `target` is `null` after the viewer loses focus or the
selected record disappears.

Provider snapshots are transient. Reapply them after a cold server start. The
`ping` response advertises support as `capabilities.agent_provider`.
...
## Receipted agent startup

On Linux and macOS with a supported POSIX shell, `agent.startup` provides
`prepare`, `launch`, and `cleanup` operations. Prepare accepts `start` (the
`agent.start` parameters) and a `preparation` string array. It writes a private
launch script and returns a receipt. Persist that receipt before sending
`launch` with the unchanged receipt. Timeout values must be greater than 3000 ms
and at most 300000 ms; invalid values are rejected during prepare.

The daemon submits only a short source command to the terminal. Accepted input
is never submitted again for the same ticket. A transport timeout does not
prove that input was rejected: retain the receipt and do not replay the launch.

Send `cleanup` with the original receipt to recover a failed startup. Cleanup
requires the original terminal binding and shell lifetime, an observed
shell-only foreground and session, and no conflicting agent ownership. Once a
submitted script returns, its completion marker permits these checks. A missing
completion marker remains uncertain. Externally closed terminals require
absence of the original terminal runtime, attachment, and shell PID.

Successful cleanup immediately retires the private launch files. The issuing
daemon retains the completed receipt for 24 hours, so an exact cleanup retry
returns `state: "cleaned"` if the response was lost. This acknowledges the
previous operation without closing resources again or requiring the old PID to
remain absent. A rebound agent name is refused. Expiration is checked on later
startup requests and retries do not extend it. External-close tombstones are
separate from completed cleanup proof and still require the absence checks.

Receipts are held in daemon memory. Unknown, modified, expired, or pre-restart
receipts cannot authorize cleanup. Process observations do not freeze the OS
process tree; arbitrary concurrent shell input and process creation are outside
