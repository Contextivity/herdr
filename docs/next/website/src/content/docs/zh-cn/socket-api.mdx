...
| 智能体 | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.view.set`、`agent.view.clear`、`agent.provider.replace`、`agent.provider.clear`、`agent.provider.get`、`agent.provider.snapshot`、`agent.provider.focus`、`agent.provider.focused` |
...
智能体提供方可以把远程或虚拟智能体加入原生 Agents 视图，而不必为每个智能体
创建本地工作区、窗格或 PTY。提供方记录沿用本地智能体的 status、元数据 token、
筛选、排序和编排卡片。`agent.list` 仍只返回本地实体智能体，因此一个提供方不会
再次采集另一个提供方投影出的记录。

`agent.provider.replace` 会原子替换某个 source 的完整快照。revision 从 1 开始并且
必须递增。viewer 必须指向一个仍然存在的本地窗格。每个 source 最多包含 2,000 条
记录；每条记录最多包含 8 个 state label 和 32 个元数据 token。元数据键最多使用
32 个 ASCII 字母、数字、下划线或连字符，值最多包含 256 个字符:
...
使用 `agent.provider.get` 读取一条 source/id 记录，使用
`agent.provider.snapshot` 读取整个 source 快照。要清空 source，提交更大的
tombstone revision。清空后仍可读取该快照；响应会保留 revision，省略 viewer，
并返回空的 `agents` 列表:
...
`agent.provider.focus` 会聚焦 source 共用的 viewer 窗格，同时在侧边栏中保留所选
提供方记录的身份。`agent.provider.focused` 返回该身份。订阅
`agent.provider_focused` 可以在选择变化时重新定位 viewer；viewer 失去焦点或所选
记录消失后，事件中的 `target` 为 `null`。

提供方快照是临时状态。服务器冷启动后需要重新提交。`ping` 响应通过
`capabilities.agent_provider` 表示是否支持该接口。
